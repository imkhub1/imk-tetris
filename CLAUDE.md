...
`game.js` calls `Sfx.play(...)` at the matching game events and wires the `#sound-toggle` / `#start-sound-toggle` (mute, kept in sync) and the `#volume-slider` (now inside the pause menu). The light/dark, sound, and freeze toggles live in a vertical `.board-toggles` column beside the board. Interface buttons get `uiclick` via one delegated listener. To add a new sound, add an entry to the `sounds` map in `audio.js` and call `Sfx.play('name')` at the event site.
